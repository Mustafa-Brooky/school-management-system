...
function renderDashboard() {
  ...
  const students = StorageService.getData(APP_KEYS.STUDENTS, []);
  const teachers = StorageService.getData(APP_KEYS.TEACHERS, []);
  const classes = StorageService.getData(APP_KEYS.CLASSES, []);
  const subjects = StorageService.getData(APP_KEYS.SUBJECTS, []);
  const grades = StorageService.getData(APP_KEYS.GRADES, []);

  const statsGrid = document.getElementById("stats-grid");
  if (statsGrid) {
    statsGrid.innerHTML = `
      <article class="stat-card">
        <div class="stat-card-icon"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg></div>
        <h3>Total Students</h3>
        <p>${students.length}</p>
        <div class="stat-card-change positive">+12% from last month</div>
      </article>
      <article class="stat-card">
        <div class="stat-card-icon"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2L2 7l10 5 10-5-10-5z"/><path d="M2 17l10 5 10-5"/><path d="M2 12l10 5 10-5"/></svg></div>
        <h3>Total Teachers</h3>
        <p>${teachers.length}</p>
        <div class="stat-card-change positive">+5% from last month</div>
      </article>
      <article class="stat-card">
        <div class="stat-card-icon"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg></div>
        <h3>Total Classes</h3>
        <p>${classes.length}</p>
        <div class="stat-card-change positive">+8% from last month</div>
      </article>
      <article class="stat-card">
        <div class="stat-card-icon"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/></svg></div>
        <h3>Total Subjects</h3>
        <p>${subjects.length}</p>
        <div class="stat-card-change negative">-2% from last month</div>
      </article>
    `;
  }

  const recentActivity = document.getElementById("recent-activity");
  const activities = StorageService.getData(APP_KEYS.ACTIVITIES, []).slice(0, 5);
  if (recentActivity) {
    recentActivity.innerHTML = activities.length
      ? activities.map((a) => `<li>${a.message}</li>`).join("")
  ...
  const byStudent = students.map((student) => {
    ...
    const avg = studentGrades.length
      ? studentGrades.reduce((sum, item) => sum + Number(item.grade), 0) / studentGrades.length
      : 0;
  ...
  if (topStudents) {
    const top = byStudent
      ...
      .slice(0, 5);
    topStudents.innerHTML = top.length
      ? top.map((item) => `<li>${item.name} (${item.average.toFixed(1)})</li>`).join("")
      : "<li>No grade data.</li>";
  }

  const failingStudents = document.getElementById("failing-students");
  if (failingStudents) {
    const failing = byStudent.filter((item) => item.average > 0 && item.average < 50);
    failingStudents.innerHTML = failing.length
      ? failing.map((item) => `<li>${item.name} (${item.average.toFixed(1)})</li>`).join("")
      : "<li>No failing students.</li>";
  }
}
